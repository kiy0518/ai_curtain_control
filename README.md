...
| 손 모양 | 인식 | 화면 표시 | 조건 (검지·중지·약지·새끼) |
|---------|------|-----------|------|
| ✋ 손바닥 | OPEN | **열림**(초록) | 4개 펴짐 (n≥4) |
| ✊ 주먹 | CLOSE | **닫힘**(빨강) | 0~1개 펴짐 (n≤1) |
| ✌️ 브이 | STOP | **정지**(주황) | 검지+중지만 |

- **펴짐 판정**: 손가락 끝(tip)이 중간관절(pip)보다 손목에서 멀면 "펴짐" — 손 방향과 무관.
- **엄지 제외**(가장 부정확).
- **떨림 방지**: 같은 제스처가 5프레임 연속 유지돼야 표시 (`GestureStabilizer`).
